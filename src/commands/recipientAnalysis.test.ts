...
describe('analyzeRecipients', () => {
  ...
  it('ignores trusted domains when flagging outliers and keeps default primary domain', () => {
    const analysis = analyzeRecipients(recipients, {
      trustedDomains: ['beta.com'],
      defaultPrimaryDomain: 'sender@alpha.com'
    });

    expect(analysis.primaryDomain).toBe('alpha.com');
    expect(analysis.outliers.map((r) => r.emailAddress)).not.toContain('carol@beta.com');
    expect(analysis.trustedDomains).toEqual(['beta.com']);
    expect(analysis.defaultPrimaryDomain).toBe('alpha.com');
  });
...
describe('formatSummary', () => {
  it('summarizes analysis into a readable string', () => {
    ...
    const analysis = analyzeRecipients(recipients, { trustedDomains: ['beta.com'] });
    ...
    expect(summary).toContain('Trusted domains ignored: beta.com');
    expect(summary).toContain('No domain outliers detected.');
